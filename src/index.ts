...
export class Mustr extends EventEmitter implements IMustr {
  ...
  render(name: string | ITemplate, output?: string | IRegisterConfig | NodeCallback | boolean, options?: IRegisterConfig | NodeCallback | boolean, force?: boolean | NodeCallback, done?: NodeCallback, group?: any) {
    ...
    if (_isString(name) && this.components[name]) {
      ...
      rollbackDir = join(this.options.configDir, 'rollbacks', rollbackId);
      ...
      const parsedPath = parse(template.outputPath as string);
      const parsedOutput = parse(this.outputPath);

      // Ensure the directory structure exists.
      ensureDirSync(parsedPath.dir);
      ...
      template.injects.forEach((inj: IInject) => {
        ...
        if (inj.relative) {
          resolvedInject = resolve(parsedPath.dir, inj.filename);
        ...
        let relTo = relative(parsedInject.dir, template.outputPath);

        const parsedTo = parse(relTo);
        let parsedName = parsedTo.name;
        if (/index$/.test(parsedName))
          parsedName = '';

        relTo = join(parsedTo.dir, parsedName);
        relTo = !/^\./.test(relTo) ? './' + relTo : relTo;

        let metadata = _extend({}, template.metadata);
        metadata.$component.from = relTo;

        let i = inj.insert.length;

        while (i--) {

          inj.insert[i] = this.renderer(inj.insert[i], metadata);
      ...
      template.beforeRender(template, () => {
        ...
        let outputName = this.transformCase(parsedPath.name, template.filenameCasing || 'lower');

        let outputPath = join(parsedPath.dir, outputName + parsedPath.ext);
        ...
        if (template.noOutput) {
          ...
          if (existsSync(outputPath)) {
            ...
            const rollbackFrom = join(rollbackDir, Date.now() + '-' + outputName + parsedPath.ext);
  ...
  removeRollbacks(by: string | number | Date, save?: boolean): IMustr {
    ...
    const removeByKey = (key) => {
      ...
      const rollbackPath = join(this.options.configDir, 'rollbacks', key);
  ...
  reindexRollbacks(prune?: boolean): IMustr {
    ...
    const keys = _keys(this.rollbacks).sort();
    const dirs = this.getDirs(resolve(this.options.configDir, 'rollbacks'));
    ...
    if (prune !== false) {
      ...
      dirs.forEach((d) => {
        ...
        if (keys.indexOf(d) === -1)
          removeSync(resolve(this.options.configDir, 'rollbacks', d));
