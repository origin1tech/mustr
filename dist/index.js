...
var Mustr = (function (_super) {
    ...
    function Mustr(options) {
        ...
        _this.loaded = false;
        _this.rollbackIdExp = /^[0-9]+-[a-z0-9]+$/;
        // Utils
    ...
    Mustr.prototype.configure = function (name, output, options) {
        ...
        delete attrs.config;
        template.metadata = fmatter.attributes;
        ...
        if (template.outputPath) {
            ...
            if (!parsedOutput_1.ext) {
                this.log.error('failed to configure template using extension of undefined.');
                return;
            }
            template.metadata.$component = {
    ...
    Mustr.prototype.render = function (name, output, options, force, done, group) {
        ...
        if (lodash_1.isString(name) && this.components[name]) {
            ...
            if (!group) {
                ...
                rollbackId = group.id + '-' + group.component;
            }
            // Ensure force.
            ...
            rollbackDir = path_1.join(this.options.configDir, 'rollbacks', rollbackId);
            var partials_1 = {};
            ...
            var parsedPath_1 = path_1.parse(template.outputPath);
            var parsedOutput = path_1.parse(this.outputPath);
            // Ensure the directory structure exists.
            fs_extra_1.ensureDirSync(parsedPath_1.dir);
            var injects_1 = [];
            var success_1 = 0;
            var failed_1 = 0;
            // Iterate the inject
            template.injects.forEach(function (inj) {
                ...
                if (inj.relative) {
                    resolvedInject = path_1.resolve(parsedPath_1.dir, inj.filename);
                ...
                // We need to get the relative path to
                // the source doc we're injecting from.
                var relTo = path_1.relative(parsedInject.dir, template.outputPath);
                var parsedTo = path_1.parse(relTo);
                var parsedName = parsedTo.name;
                if (/index$/.test(parsedName))
                    parsedName = '';
                relTo = path_1.join(parsedTo.dir, parsedName);
                relTo = !/^\./.test(relTo) ? './' + relTo : relTo;
                var metadata = lodash_1.extend({}, template.metadata);
                metadata.$component.from = relTo;
                var i = inj.insert.length;
                while (i--) {
                    inj.insert[i] = _this.renderer(inj.insert[i], metadata);
                }
            ...
            template.beforeRender(template, function () {
                var outputName = _this.transformCase(parsedPath_1.name, template.filenameCasing || 'lower');
                var outputPath = path_1.join(parsedPath_1.dir, outputName + parsedPath_1.ext);
                // Render the template.
                ...
                if (template.noOutput) {
                    ...
                    if (fs_extra_1.existsSync(outputPath)) {
                        ...
                        var rollbackFrom = path_1.join(rollbackDir, Date.now() + '-' + outputName + parsedPath_1.ext);
    ...
    Mustr.prototype.removeRollbacks = function (by, save) {
        ...
        var removeByKey = function (key) {
            ...
            var rollbackPath = path_1.join(_this.options.configDir, 'rollbacks', key);
    ...
    Mustr.prototype.reindexRollbacks = function (prune) {
        var _this = this;
        var keys = lodash_1.keys(this.rollbacks).sort();
        var dirs = this.getDirs(path_1.resolve(this.options.configDir, 'rollbacks'));
        var tmp = {};
        ...
        if (prune !== false) {
            dirs.forEach(function (d) {
                ...
                if (keys.indexOf(d) === -1)
                    fs_extra_1.removeSync(path_1.resolve(_this.options.configDir, 'rollbacks', d));
            });
        }
    ...
    Mustr.prototype.transformCase = function (str, to) {
        ...
        if (!caseMap[to])
            this.log.error("invalid transform case " + to + ".").exit();
